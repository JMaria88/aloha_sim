...
container_position = distributions.Uniform(
    low=[-0.18, -0.1, TABLE_HEIGHT + RESET_HEIGHT],
    high=[-0.12, 0.1, TABLE_HEIGHT + RESET_HEIGHT],
    single_sample=True,
)
...
class HandOverTaskConfig:
  """Configuration for a handover task.
...
HANDOVER_CONFIGS = {
    ...
    'banana': HandOverTaskConfig(
        ...
        instruction='hand over the banana and put it in the bowl',
    ),
    # Hands over a pen and puts it in container.
    'pen': HandOverTaskConfig(
        ...
        overlap_boxes=[
            ...
            # Larger box above the container, designed to catch the tail of the
            # pen as it fans out from the top of the container.
            oobb_utils.Oobb(
        ...
        instruction='hand over the pen and put it in the container',
    ),
}


class HandOverLeftLifted(aloha2_task.AlohaTask):
  """Hand over task.

  The goal is to pick up the object with one gripper, hands to another gripper,
  and put the object in a container.
  """
  ...
  def __init__(
      ...
      reward_requires_handover=False,
      **kwargs,
  ):
    """Initializes a new `HandOver` task.

    Args:
      object_name: str specifying the name of the object to be handed over.
      reward_based_on_overlap: Whether to reward based on overlap between the
        object and the container.
      reward_requires_handover: Whether success requires a handover or not.
    ...
    self._reward_requires_handover = reward_requires_handover
    if not self._reward_requires_handover:
      # Don't require states where robot touches the objects.
      self._success_state = 2

    if object_name not in HANDOVER_CONFIGS.keys():
      raise ValueError(
          f'Invalid object name: {object_name}, must be one of'
          f' {HANDOVER_CONFIGS.keys()}'
      )
    task_config = HANDOVER_CONFIGS[object_name]
  ...
  def get_reward(self, physics):
    ...
    left_gripper_geom_ids = list(
        physics.bind(
            self.root_entity.mjcf_model.find(
                'body', r'left\gripper_link'
            ).find_all('geom')
        ).element_id
    )
    ...
    if self._success_state == 0:
      ...
      if _touching(left_gripper_geom_ids, object_geom_ids):
        self._success_state = 2
    elif self._success_state == 2:
